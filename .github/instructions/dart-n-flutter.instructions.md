---
description: 'When editing existing code, apply these rules only to the lines you add or modify. Do not silently reformat surrounding code.'
applyTo: '**/*.dart'
---

# Dart and Flutter

If a rule in this document conflicts with a rule enforced by the project's `analysis_options.yaml`, follow `analysis_options.yaml` and note the conflict in a comment. Do not suppress analyzer warnings without being explicitly asked.
Do not apply any of these rules to generated files (files ending in `.g.dart`, `.freezed.dart`, `.mocks.dart`, or any file containing the comment `// GENERATED CODE - DO NOT MODIFY BY HAND`). If asked to edit such a file, explain that it is generated and should be regenerated via its build tool instead.
If you observe a violation in existing code in the same top-level function body or class declaration (including all its members) as your change, add a comment of the form `// TODO: [rule name] violation - [brief description]` on the line containing the violation. Do not descend into nested classes or functions outside the immediate declaration you are editing.
...
*   PREFER relative import paths for local files within the same package. Use `package:` imports for external packages, and keep import ordering as `dart:`, then `package:`, then relative.
...
* Write unit tests for every service, repository, and ViewModel class covering all methods that contain custom logic. Exclude methods whose entire body is a single framework super-call, auto-generated code, or trivial field access (for example, a getter that returns a field directly).
* If the file being edited does not contain a class identifiable as a service, repository, or ViewModel (for example, it is a standalone script, a `main.dart` entry point, or a platform interface), skip the testing recommendations in this section and apply only the Effective Dart style, documentation, usage, and design rules.
* Write widget tests for views. A "view" for testing purposes is any Widget class whose name ends in `Screen`, `Page`, or `View`, or any widget that is registered as a named route. Small reusable component widgets should be tested only when they contain non-trivial internal state or logic. Testing routing and dependency injection are particularly important.
